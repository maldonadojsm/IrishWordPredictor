# !/usr/bin/env python
# title           :BigramKatzLaplaceSmoothing.py
# description     :This will create a header for a python script.
# author          :Juan Maldonado
# date            :4/11/19
# version         :1.0
# usage           :python3 BigramKatzLaplaceSmoothing.py
# notes           :
...
cprob_u_l = nltk.LaplaceProbDist(freq_unigram)
...
cprob_laplace = nltk.ConditionalProbDist(count_freq_bigram, nltk.LaplaceProbDist)
        ...
        for i in test_lines:
            if wordPairs[c] in i:
                ...
                try:
                    ...
                    prob_ascii_bl = unigram_index_min_1 * cprob_laplace[index_min_1].prob(ascii[a])

                    prob_tilde_bl = unigram_index_min_1 * cprob_laplace[index_min_1].prob(tilde[b])


                    # NORMALIZE: N
                    bigram_prob_bl = prob_ascii_bl / (prob_ascii_bl + prob_tilde_bl)


                    # Calculate Bi Gram Prob

                    bigram_prob = (bigram_prob_bl)

                    # BACK OFF
                    if bigram_prob_bl == 1 or bigram_prob_bl == 0:
